...
import {
  ...
  IonIcon, IonButton, IonList, IonSpinner
...
@Component({
    ...
    <ion-content class="ion-padding">
      ...
      <div *ngIf="!loading && client">
        ...
        <ion-card>
          ...
          <ion-card-content>
            <div *ngIf="client.company_name" class="info-row">
        ...
        <ion-card class="actions-card">
          ...
          <ion-card-content>
            ...
            <div *ngIf="!client.phone && !client.email && !client.address" class="no-info">
        ...
        <ion-card *ngIf="events.length > 0">
          <ion-card-header>
            <ion-card-title>🎉 Événements ({{ events.length }})</ion-card-title>
          </ion-card-header>
          <ion-card-content>
            <div *ngFor="let evt of events" class="event-mini">
              <strong>{{ evt.name }}</strong>
              <span>{{ formatDate(evt.start_date) }} · {{ evt.location || '-' }}</span>
            </div>
          </ion-card-content>
        </ion-card>
      </div>
  ...
  styles: [`
    ...
    .info-row { display: flex; align-items: center; gap: 10px; color: #555; 
      ion-icon { color: #f39c12; } 
    }
    .actions-card ion-card-content { padding: 8px 16px; }
    .action-btn {
      display: flex; align-items: center; gap: 14px; padding: 16px;
      border-radius: 12px; margin-bottom: 10px; text-decoration: none; cursor: pointer;
      transition: transform 0.1s;
      &:active { transform: scale(0.98); }
      ion-icon { font-size: 1.6rem; min-width: 40px; height: 40px; border-radius: 50%;
        display: flex; align-items: center; justify-content: center; color: white; }
      div { display: flex; flex-direction: column;
        strong { font-size: 0.95rem; }
        span { font-size: 0.8rem; color: #777; margin-top: 2px; }
      }
    }
    .action-call { background: #e8f8f0;
      ion-icon { background: #27ae60; }
      strong { color: #27ae60; }
    }
    .action-email { background: #eaf2fe;
      ion-icon { background: #3498db; }
      strong { color: #3498db; }
    }
    .action-map { background: #fef5e7;
      ion-icon { background: #f39c12; }
      strong { color: #e67e22; }
    }
    .no-info { text-align: center; color: #999; padding: 20px; }
    .event-mini {
      padding: 10px 0; border-bottom: 1px solid #f0f0f0;
      strong { display: block; font-size: 0.9rem; color: #2c3e50; }
...
export class ClientDetailPage implements OnInit {
  ...
  loadClient(id: string) {
    this.http.get<any>(`${environment.apiUrl}/clients/read_detail.php?id=${id}`).subscribe({
      next: (r) => {
        this.client = r.data?.client || r.data;
  ...
  getMapsUrl(): string {
    const addr = encodeURIComponent(this.client.address || '');
