...
@Component({
    ...
    <ion-content class="ion-padding">
      ...
      <div *ngIf="!loading && event">
        <ion-card>
          ...
          <ion-card-content>
            <div class="info-row"><ion-icon name="calendar-outline"></ion-icon><span>{{ formatDate(event.start_date) }} → {{ formatDate(event.end_date) }}</span></div>
            <div class="info-row" *ngIf="event.location"><ion-icon name="location-outline"></ion-icon><span>{{ event.location }}</span></div>
        ...
        <ion-card>
          ...
          <ion-card-content>
            <div class="note-form">
              <ion-textarea [(ngModel)]="newNote" placeholder="Ajouter une note rapide..." [rows]="3" fill="outline"></ion-textarea>
  ...
  styles: [`
    ...
    .info-row { display:flex; align-items:center; gap:10px; padding:8px 0; border-bottom:1px solid #f0f0f0; font-size:0.9rem; color:#555; ion-icon { color:#f39c12; font-size:1.1rem; min-width:20px; } }
    .client-link { cursor:pointer; color:#3498db; small { margin-left:auto; color:#999; font-size:0.75rem; } &:active { background:#f0f7ff; } }
    ...
    .note-item { background:#f8f9fa; border-radius:8px; padding:12px; margin-bottom:10px;
      .note-header { display:flex; justify-content:space-between; margin-bottom:6px; strong { font-size:0.85rem; color:#2c3e50; } .note-date { font-size:0.75rem; color:#999; } }
...
export class EventDetailPage implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private router: Router, private http: HttpClient, private auth: AuthService) { addIcons({ calendarOutline, locationOutline, personOutline, addOutline }); }
  ngOnInit() { const id = this.route.snapshot.paramMap.get('id'); if (id) this.loadEvent(id); }
  loadEvent(id: string) {
