...
import {
  ...
  IonBadge, IonRefresher, IonRefresherContent, IonSpinner, IonNote, IonIcon
} from '@ionic/angular/standalone';
import { addIcons } from 'ionicons';
import { locationOutline, calendarOutline, chevronForwardOutline } from 'ionicons/icons';
...
@Component({
  ...
  imports: [
    ...
    IonLabel, IonBadge, IonRefresher, IonRefresherContent, IonSpinner, IonNote, IonIcon
  ],
  template: `
    <ion-header>
      <ion-toolbar>
        <ion-title>📅 Événements à venir</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content>
      ...
      <ion-list *ngIf="!loading && events.length > 0" lines="none" class="event-list">
        <ion-item *ngFor="let event of events" (click)="openEvent(event)" button detail class="event-card">
          ...
          <ion-badge slot="end" [color]="getStatusColor(event.status)">
  ...
  styles: [`
    ...
    .event-card {
      --background: white; --border-radius: 12px; --padding-start: 16px;
...
export class EventsPage implements OnInit {
  ...
  constructor(private http: HttpClient, private router: Router, private auth: AuthService) {
    addIcons({ locationOutline, calendarOutline, chevronForwardOutline });
  }
