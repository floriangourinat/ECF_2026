...
export class QuoteRequestComponent {
  ...
  loadSuccessMessage(): void {
    this.http.get<any>('/api/settings/get_quote_success_message.php')
      .subscribe({
        next: (response) => {
          if (response?.message) {
  ...
  onSubmit(): void {
    ...
    this.http.post<any>('/api/prospects/create.php', this.quoteForm.value)
      .subscribe({
        next: (response) => {
  ...
  uploadImage(prospectId: number): void {
    ...
    this.http.post<any>('/api/prospects/upload_image.php', formData)
      .subscribe({
        next: () => {
